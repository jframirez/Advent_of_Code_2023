...
int main(void) {
    ...
    code_sheet.open("./sample_part2.txt", std::ios::in);
    ...
    for (const auto & node : network) {
        if (*(node.first.end() - 1) == 'A') {
            std::cout << "NODE LIST : " << node.first << std::endl;
            node_starting_points.push_back(node.first);
        }
    }

    std::cout << node_starting_points.size() << std::endl;
    ...
    if (count_z_end_match == node_starting_points.size()) {
        std::cout << "FOUND MAZE EXIT AT: " << std::endl;
    }
    ...
    while (!found) {
        for (int i = 0; i < instructions.size(); ++i) {
            ...
            for (const auto & node : node_starting_points) {
                ...
                if (count_z_end_match == node_starting_points.size()) {
                    std::cout << "FOUND MAZE EXIT AT: " << instruction_count << std::endl;
                    found = true;
                    break;
                }
            }
